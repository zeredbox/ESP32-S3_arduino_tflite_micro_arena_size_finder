...
### 📄 Include your Edge Impulse project

Replace `YOUR_LIBRARY` with the exact name of the Arduino library generated by your Edge Impulse project. For example, if the exported library is named `Dice`, change the line to:

```cpp
#include <Dice_inferencing.h>
```

To confirm the exact name, open:

```text
...\Arduino\libraries\<your-project-name>\src\
```

Then declare YOUR arena size symbol (found in your exported model header, under:
      ...\Arduino\libraries\<your-project-name>\src\tflite-model\tflite_learn_*.h).

Example: extern const size_t tflite_learn_1077716_45_arena_size;
Modify the line extern const size_t tflite_learn_XXXXXXX_ZZ_arena_size;   // <-- CHANGE THIS
to
